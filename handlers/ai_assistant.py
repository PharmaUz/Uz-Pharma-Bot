...
def check_daily_limit(user_id: int) -> bool:
    ...
    # Check limit
    return user_daily_limits[user_id]["count"] < DAILY_LIMIT
...
def format_ai_response_for_telegram(text: str) -> str:
    ...
    for line in lines:
        ...
        if any(word in line.lower() for word in ['muhim', 'diqqat', 'eslatma', 'ogohlantirish']):
...
async def process_ai_question(message: types.Message, state: FSMContext):
    """Processes question sent to AI - ONLY in AI consultation state"""
    ...
    # Clear state after processing AI response
    await state.clear()
